document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchLeaderboard(year = "current") {
        ...
        fetch(url)
            ...
            .then(data => renderLeaderboard(data, year))
            .catch(error => {
    ...
    function renderLeaderboard(data, year) {
        ...
        const bodyRows = data.MRData.StandingsTable.StandingsLists[0].DriverStandings.map(driver => {
            let circleClass = "";
            if (driver.position === "1") circleClass = "glowing-circle first-place";
            if (driver.position === "2") circleClass = "glowing-circle second-place";
            if (driver.position === "3") circleClass = "glowing-circle third-place";

            return `
                <tr>
                    <td><span class="${circleClass}">${driver.position}</span></td>
    ...
    setTimeout(() => {
        setInterval(() => {
            if (isDynamic) {
                ...
                console.log("Removed .dynamic class");
            } else {
                mainElement.classList.add("dynamic");
                console.log("Added .dynamic class");
            }
            isDynamic = !isDynamic;
        }, 2000);
    ...
    let showingLeft = true; // Start with the left image

    toggleImage.addEventListener("click", function () {
        if (showingLeft) {
            toggleImage.src = "assets/images/spdmRight.png"; // Update the image source
            leftText.classList.remove("active-text");
            rightText.classList.add("active-text");
        } else {
            toggleImage.src = "assets/images/spdmLeft.png"; // Update the image source
    ...
    function startUpload() {
        ...
        setTimeout(completeUpload, uploadDuration * 1000);
    }
